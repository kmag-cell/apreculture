...
class FertilizerPage extends StatefulWidget {
  ...
  static const routeName = '/fertilizer';
...
class _FertilizerPageState extends State<FertilizerPage> {
  ...
  void _fertilizerNew() async {
    var isValid = _FormKey.currentState!.validate();
    if (!isValid) {
      return;
    }
    _FormKey.currentState!.save();
    dynamic isfertilizer = await Provider.of<FertilizerState>(
      context,
      listen: false,
    ).fertlizer(_culture, _nitrogen, _phosphor, _potassium);
    if (isfertilizer != null) {
      print(isfertilizer);
      Navigator.of(context)
          .pushNamed(Fertilizer1Page.routeName, arguments: isfertilizer);
    } else if (isfertilizer["error"]) {
      showDialog(
          context: context,
          builder: (context) {
            return AlertDialog(
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      Container(
                                        child: TextFormField(
                                          decoration: ThemeHelper()
                                              .textInputDecoration('Culture',
                                                  'Entrez le nom de la culture'),
                                          validator: (value) {
                                            if (value!.isEmpty) {
